...
class Sampler:
    ...
    def add_doc(self, doc, vocab, doc_id = None,
        ...
        for ww in temp_doc:
            ...
            self._doc_assign[doc_id].append(assignment)   # topic assignment of each term in the doc
    ...
    def change_topic(self, doc, index, new_topic):
        ...
        if old_topic != -1:
            ...
            self._topics._topic_term[old_topic][term] -= 1
            ### decrease count of
        ...
        if new_topic != -1:
            ...
            self._topics._topic_term[new_topic][term] += 1
            ### increase count of self._topic_term[new_topic][term]
    ...
    def sample_probs(self, doc_id, index):
        ...
        for kk in range(self._num_topics):
            ...
            for ii in range(self._num_topics):
                    t_doc += self._doc_counts[doc_id][ii]
            for ii in range(len(self._vocab)):
                   t_topic += self._topics._topic_term[kk][ii]

            first = ((self._doc_counts[doc_id][kk] + self._alpha[kk]) * 1.0)/(t_doc + (self._num_topics * 1))
            second = ((self._topics._topic_term[kk][term] + 1) * 1.0)/(t_topic + (len(self._vocab) * 1))
            #second = LdaTopicCounts.word_in_topic(kk, 0)
            sample_probs[kk] = first * second
