...
from math import exp, log, pow, floor
...
class LogReg:
    ...
    def best_worst_features(self):
        # Best/worst predictors
        min_negative = max([n for n in self.beta if n<0])
        max_negative = min([n for n in self.beta if n<0])
        max_positive = max([n for n in self.beta if n>0])
        min_positive = min([n for n in self.beta if n>0])
        min_negative_index = list(self.beta).index(min_negative)
        max_negative_index = list(self.beta).index(max_negative)
        min_positive_index = list(self.beta).index(min_positive)
        max_positive_index = list(self.beta).index(max_positive)
        return vocab[max_negative_index], vocab[min_negative_index], vocab[min_positive_index], vocab[max_positive_index]
    ...
    def sg_update(self, train_example, iteration,
        ...
        pi = exp(np.transpose(self.beta).dot(train_example.x))/(1 + exp(np.transpose(self.beta).dot(train_example.x)))
        update = self.step(iteration) * ((-1 * self.mu * self.beta) + train_example.x * (train_example.y - pi))
        self.beta = self.beta + update
        return self.beta

        # if lazy == False:
        #     pi = exp(np.transpose(self.beta).dot(train_example.x))/(1 + exp(np.transpose(self.beta).dot(train_example.x)))
        #     update = self.step(iteration) * (train_example.x * (train_example.y - pi))
        #     self.beta = self.beta + update
        #     self.last_update.fill(iteration)
        # else:
        #     if np.any(train_example.x) == True:
        #         pi = exp(np.transpose(self.beta).dot(train_example.x))/(1 + exp(np.transpose(self.beta).dot(train_example.x)))
        #         x_update = train_example.x * (train_example.y - pi)
        #         self.beta = self.beta + x_update
    ...
    def finalize_lazy(self, iteration):
        ...
        """
        # self.beta = self.beta * pow((1 - (2 * self.mu * self.step(iteration))), iteration - self.last_update[0])
        # self.last_update.fill(iteration)
        # return self.beta
...
if __name__ == "__main__":
    ...
    if args.ec != "rate":
        ...
        # Exponential Decay, where decay is 0.5 and the learning rate is updated every 10 iterations
        lr = LogReg(len(vocab), args.mu, lambda x: args.step * exp(-0.01*floor(x/10)))
    ...
    print("Update %i\tTP %f\tHP %f\tTA %f\tHA %f" %
          (update_number, train_lp, ho_lp, train_acc, ho_acc))
    best_negative_class, worst_negative_class, worst_positive_class, best_positive_class = lr.best_worst_features()
    #print("best_negative_class: ", best_negative_class, "worst_negative_class: ", worst_negative_class, "worst_positive_class: ", worst_positive_class, "best_positive_class: ", best_positive_class)
