...
def vectorize(ex, word2ind):
    ...
    for i in range(len(question_text)):
        try:
            vec_text[i] = word2ind[question_text[i]]
        except KeyError:
...
def evaluate(data_loader, model, device):
    ...
    num_examples = 0
    error = 0
    with open('./data/question_dev_cl1.json') as json_data:
        questions = json.load(json_data)
    ...
    for idx, batch in enumerate(data_loader):
        ...
        labels = batch['labels']
        ####Your code here
        logits = model(torch.LongTensor(question_text), torch.tensor(question_len))
        top_n, top_i = logits.topk(1)

        error += torch.nonzero(top_i.squeeze() - torch.LongTensor(labels)).size(0)
        # This is for fetching the sentences which were classified wrong
        # ab = top_i.squeeze() - torch.LongTensor(labels)
        # print(ab)
        # for i in range(len(ab)):
        #     if ab[i] < 0:
        #         print(questions[ (num_examples+ i)]['question_text'])
...
def train(args, model, train_data_loader, test_data_loader, accuracy, device):
    """
    ...
    for idx, batch in enumerate(train_data_loader):
        ...
        #### Your code here
        model.zero_grad()
        out = model(torch.LongTensor(question_text), torch.tensor(question_len))
        batch_loss = criterion(out, batch['labels'])
        batch_loss.backward()
        optimizer.step()

        clip_grad_norm_(model.parameters(), args.grad_clipping)
        print_loss_total += batch_loss.data.numpy()
        epoch_loss_total += batch_loss.data.numpy()
        ...
        if idx % args.checkpoint == 0 and idx > 0:
            ...
            print_loss_total = 0
            curr_accuracy = evaluate(test_data_loader, model, device)
...
if __name__ == "__main__":
    ...
    ###### For GloVe encoding, creating of weight matrix
    # words = []
    # idx = 0
    # word2idx = {}
    ...
    #             vectors.append(vect)
    # vectors = bcolz.carray(vectors[1:].reshape((len(voc), 50)), rootdir='6B.50.dat', mode='w')
    # vectors.flush()
    #
