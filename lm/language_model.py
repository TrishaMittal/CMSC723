...
class BigramLanguageModel:
    ...
    def __init__(self, unk_cutoff, jm_lambda=0.6, dirichlet_alpha=0.1,
        ...
        # Add your code here!
        self.counter = dict()
        self.context_word_counter = defaultdict(lambda: 0)
        self.word_counter = defaultdict(lambda: 0)
    ...
    def train_seen(self, word, count=1):
        ...
        if count:
            #print("entered", count)
            self.counter[word] = self.counter[word] + count
        else:
            self.counter[word] += 1
    ...
    def vocab_lookup(self, word):
        ...
        if word == kSTART or word == kEND:
            return word
        try:
            if self.counter[word] < self._unk_cutoff:
                return 'UNK'
            else:
                return word
        except KeyError:
    ...
    def mle(self, context, word):
        ...
        if (float(self.context_word_counter[(context,word)])/float(self.word_counter[context]) == 0.0):
            return kNEG_INF
        else:
            return lg(float(self.context_word_counter[(context,word)])/float(self.word_counter[context]))
    ...
    def laplace(self, context, word):
        ...
        # This initially return 0.0, ignoring the word and context.
        # Modify this code to return the correct value.
        return lg(float(self.context_word_counter[(context,word)] +1.0)/float(self.word_counter[context]+ len(self.word_counter)))
    ...
    def jelinek_mercer(self, context, word):
        ...
        # This initially return 0.0, ignoring the word and context.
        # Modify this code to return the correct value.
        return lg((self._jm_lambda * float(self.context_word_counter[(context, word)]/self.word_counter[context])) + ((1 - self._jm_lambda) * float(self.word_counter[word]/float(len(self.word_counter)))))
    ...
    def kneser_ney(self, context, word):
        ...
        # This initially return 0.0, ignoring the word and context.
        # Modify this code to return the correct value.
        vocab = float(1/float(len(self.word_counter)))
        counter_no_start = sum(self.word_counter.values()) - self.word_counter[kSTART]
        counter_no_start_unique = len(self.word_counter) - 1
        unigram_calc = float(max(0,(self.word_counter[word] - self._kn_discount))/(self._kn_concentration + counter_no_start)) + (float((self._kn_concentration + (float(counter_no_start_unique * self._kn_discount)))/(self._kn_concentration + counter_no_start)) * vocab)
        counter_for_bigram = 0
        counter_for_bigram_unique = 0
        for i in self.word_counter.keys():
            counter_for_bigram += self.context_word_counter[(context, i)]
            if self.context_word_counter[(context, i)] != 0:
                counter_for_bigram_unique += 1
        bigram_calc = float(max(0,(self.context_word_counter[(context, word)] - self._kn_discount))/(self._kn_concentration + counter_for_bigram)) + (float((self._kn_concentration + (counter_for_bigram_unique * self._kn_discount))/(self._kn_concentration + counter_for_bigram)) * unigram_calc)
        return lg(bigram_calc)
    ...
    def dirichlet(self, context, word):
        ...
        # This initially return 0.0, ignoring the word and context.
        # Modify this code to return the correct value.
        return lg(float(self.context_word_counter[(context,word)] + self._dirichlet_alpha)/float(self.word_counter[context]+ (len(self.word_counter)* self._dirichlet_alpha)))
        #return 0.0
    ...
    def add_train(self, sentence):
        ...
        # print(self.tokenize_and_censor(sentence))
        #self._unk_cutoff = 0
        for context, word in bigrams(self.tokenize_and_censor(sentence)):
            try:
                self.context_word_counter[(context,word)] += 1
            except KeyError:
                self.context_word_counter[(context,word)] = 1
...
if __name__ == "__main__":
    ...
    for ii in nltk.corpus.treebank.sents():
        jj = lm._normalizer(" ".join(ii))
        test_sentence_count += 1
        #print(test_sentence_count, jj)
        #print("#".join(str(x) for x in lm.tokenize_and_censor(jj)))
        #print(lm.perplexity(jj, getattr(lm, args.method)))
        sentence_dict[jj] = lm.perplexity(jj, getattr(lm, args.method))
    a = sorted(sentence_dict.items(), key=itemgetter(1))[:10]
    for i in a:
        print(i)
